...
export class Hud {
  ...
  update(sim: Simulation, fps?: number) {
    ...
    const nat = NATIONS[sim.playerNation];
    this.resourcesEl.textContent =
      `Food ${Math.floor(r.food)} · Timber ${Math.floor(r.timber)} · Metal ${Math.floor(r.metal)} · Wealth ${Math.floor(r.wealth)} · Knowledge ${Math.floor(r.knowledge)}`;

    const epoch = sim.getEpoch();
    this.epochEl.textContent = `${nat.name} · ${epoch.name}`;
